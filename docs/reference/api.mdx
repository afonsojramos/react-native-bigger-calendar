...
## Selection (month mode)

See the [Date selection](/guides/selection) guide.

| Prop             | Type                | Notes                                           |
| ---------------- | ------------------- | ----------------------------------------------- |
| `selectedDate`   | `Date \| null`      | Mark a single day selected.                     |
| `selectedDates`  | `Date[]`            | Mark several days selected.                     |
| `selectedRange`  | `DateRange`         | A selected span; pair with `useDateRange`.      |
| `minDate`        | `Date`              | Earliest selectable day; earlier ones disabled. |
| `maxDate`        | `Date`              | Latest selectable day; later ones disabled.     |
| `isDateDisabled` | `(date) => boolean` | Disable specific days (dimmed, taps ignored).   |
...
<CardGroup cols={2}>
  ...
  <Card title="Helpers">
    ...
    `isWithinDateRange`, `getWeekDays`, `getIsToday`, `isWeekend`, `minutesIntoDay`
  </Card>
  <Card title="Types">
